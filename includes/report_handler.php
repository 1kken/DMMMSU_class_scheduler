...
<body>
    ...
    <?php 
        $instructor = get_instructor($pdo, $_POST['instructor_id']);
    ?>
    <div id="information">
        <table>
            <tr>
                <td>
                    <?php
                        $post_name = "";
                        if($_POST['semester_instructor'] == "1"){
                            $post_name = "st Semester";
                        }else{
                            $post_name = "nd Semester";
                        }
                    ?>
                    <h3><?php echo $_POST['semester_instructor'].$post_name.", School Year ".$_POST['sy_instructor']; ?></h3>
                </td>
            </tr>
            <tr>
                <td>
                    <h3>NAME: <?php echo $instructor['full_name']; ?> PROGRAM/YEAR/SECTION:BSCS <?php echo $_POST['section_instructor']; ?></h3>
                </td>
            </tr>
        </table>
    </div>
    <div id="sched">
        <table>
            ...
            function get_schedules_based_on_time($pdo, $time)
            {
                ...
                $stmt = $pdo->prepare($sql);
                $stmt->execute(['time' => $time, 'section_id' => $_POST['section_instructor'], 'semester' => $_POST['semester_instructor'], 'sy' => $_POST['sy_instructor'] , 'instructor_id' => $_POST['instructor_id']]);
                return $stmt->fetchAll(PDO::FETCH_ASSOC);
            }
            $time_1900 = strtotime("19:00");
            $time_0800 = strtotime("07:00");
