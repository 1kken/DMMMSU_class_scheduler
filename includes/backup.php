...
<body>
    ...
    <div id="information">
        <table>
            ...
            <tr>
                <td>
                    <h3>NAME: <?php echo $instructor['full_name']; ?> PROGRAM/YEAR/SECTION:BSCS <?php echo $_POST['section']; ?></h3>
                </td>
            </tr>
        </table>
    </div>
    <div id="sched">
        <table>
            ...
            function get_schedules_based_on_time($pdo, $time)
            {
                $sql = "SELECT subject.descriptive_title,schedule.day,schedule.start_time,schedule.end_time 
                FROM SCHEDULE JOIN SUBJECT ON schedule.subject_id = subject.subject_id WHERE start_time =:time
                AND section_id = :section_id AND schedule.semester = :semester AND sy =:sy AND :instructor_id;";
                $stmt = $pdo->prepare($sql);
                $stmt->execute(['time' => $time, 'section_id' => $_POST['section'], 'semester' => $_POST['semester'], 'sy' => $_POST['sy'] , 'instructor_id' => $_POST['instructor_id']]);
                return $stmt->fetchAll(PDO::FETCH_ASSOC);
            }
            $time_1900 = strtotime("19:00");
            $time_0800 = strtotime("07:00");
